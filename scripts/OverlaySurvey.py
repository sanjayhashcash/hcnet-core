...
def check_results(data, graph, merged_results):
    if "topology" not in data:
        raise ValueError("hcnet-core is missing survey nodes."
...
def get_tier1_stats(augmented_directed_graph):
    '''
    Helper function to help analyze transitive quorum. Must only be called on a graph augmented with HcnetBeat info
    '''
...
def augment(args):
    ...
    data = requests.get("https://api.hcnetbeat.io/v1/nodes").json()
    transitive_quorum = requests.get(
        "https://api.hcnetbeat.io/v1/").json()["transitiveQuorumSet"]
